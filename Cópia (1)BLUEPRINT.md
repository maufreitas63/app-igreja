...
| **Menu** | Volta ao índice do carrossel |
...
| **Lista de inscritos** | ✓ se `room_entry_checked` |
...
**Serve para:** CRUD de membros da família (`members`), reconhecimento familiar (aceite), herança de endereço.

| Controle | Função |
|----------|--------|
| **Formulário recolhível** | Adicionar/editar membro |
| **Busca por telefone** | Vincula perfil existente |
| **Parentesco** | Chips (Cônjuge, Filho(a), etc.) |
| **Checkbox aceite** | Reconhecimento na família |
| **Editar / Salvar / Excluir** | Por membro |
| **Banner family_id** | Somente leitura |

**Regra:** representante legal não pode ser excluído.

**Mensagens:** Acesso negado; duplicata de telefone/membro; confirmação de exclusão; RLS/accepted column errors.
...
| Controle | Função |
|----------|--------|
| **Motivo / Situação** | Modais categoria/subcategoria |
...
| **Tipos de Escala** | `MaintenanceScaleTypesCard` | ACL escala |
...
3. `scripts/escalas-integrity-constraints.sql`
4. `scripts/escalas-apply-cycle-batch.sql` (inclui `aplicar_ciclo_escala` + `get_scale_cycle_context`)
5. `scripts/escalas-volunteers-rpc.sql`
6. `scripts/escalas-maintenance-rpc.sql`
7. `scripts/checkins-totem-flow.sql` + `scripts/events-quorum-registry.sql`
8. `scripts/profiles-sync-address-from-cep-rpc.sql`
9. `scripts/access-control-map-screen.sql`
10. `scripts/financials-maintenance-rpc.sql`
11. `scripts/verificar-login.sql`, scripts PIN/LGPD conforme ambiente
