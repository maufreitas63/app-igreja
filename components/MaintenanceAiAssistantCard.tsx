...
export function MaintenanceAiAssistantCard({
  isActive = true,
  panelHeight,
  minimal = false,
}: Props) {
  ...
  const handleSaveGeminiKey = useCallback(async () => {
    ...
    try {
      ...
      Toast.show({
        ...
        text2: 'O assistente de IA já pode ser usado pela liderança.',
      });
    } catch (saveError) {
      setConfigError(
  ...
  if (canUseAssistant === null) {
    return (
      <View style={[styles.panel, { height: contentHeight }, minimal && { width: '100%' }]}>
      ...
      {activeTab === 'config' && canViewAudit ? (
        ...
        >
          ...
          <Text style={styles.metaText}>
            ...
            servidor ao atender a liderança. Você pode trocá-la quando quiser.
          </Text>
