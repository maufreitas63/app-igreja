-- =============================================================================
-- Multi-tenancy — onda 2d: relatórios de manutenção (tenant isolation)
-- =============================================================================
-- Pré-requisito: scripts/multi-tenant-wave0-helper.sql (require_session_tenant_id).
-- Fontes canônicas:
--   maintenance-reports-rpc.sql
--   maintenance-reports-active-members-age-matrix.sql
-- Nota: _report_demographic_family nao existe nas fontes canônicas (apenas _report_demographic_family_size).
-- =============================================================================

begin;

-- ---------------------------------------------------------------------------
-- _report_active_members_age_matrix
-- ---------------------------------------------------------------------------
...
grant execute on function public._report_event_registrations(jsonb) to anon;
grant execute on function public._report_event_registrations(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_financial_flow
-- ---------------------------------------------------------------------------
...
begin
  ...
  grouped as (
    ...
    and  f.transaction_date between b.month_start and b.month_end
...
grant execute on function public._report_financial_flow(jsonb) to anon;
grant execute on function public._report_financial_flow(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_members_active_inactive
-- ---------------------------------------------------------------------------
...
grant execute on function public._report_pastoral_needs(jsonb) to anon;
grant execute on function public._report_pastoral_needs(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_attendance_retention
-- ---------------------------------------------------------------------------
...
grant execute on function public._report_attendance_retention(jsonb) to anon;
grant execute on function public._report_attendance_retention(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_checkin_adoption
-- ---------------------------------------------------------------------------
...
grant execute on function public._report_checkin_adoption(jsonb) to anon;
grant execute on function public._report_checkin_adoption(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_demographic_age_brackets
-- ---------------------------------------------------------------------------
...
grant execute on function public._report_demographic_age_brackets(jsonb) to anon;
grant execute on function public._report_demographic_age_brackets(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_demographic_family
-- ---------------------------------------------------------------------------
-- NOTICE: função _report_demographic_family não encontrada em maintenance-reports-rpc.sql — pulando.
do $$ begin raise notice 'wave2: skipped missing function _report_demographic_family'; end $$;

-- ---------------------------------------------------------------------------
-- _report_demographic_family_size
-- ---------------------------------------------------------------------------
...
grant execute on function public._report_demographic_family_size(jsonb) to anon;
grant execute on function public._report_demographic_family_size(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_digital_adoption
-- ---------------------------------------------------------------------------
...
begin
  ...
  select
    ...
    jsonb_build_object(
      ...
      'sessoes', (select count(*) from public.profile_app_access_events where tenant_id = v_tenant
    and accessed_at >= v_cutoff),
      'inscricoes_eventos', (select count(*) from public.event_registrations where tenant_id = v_tenant
...
grant execute on function public._report_digital_adoption(jsonb) to anon;
grant execute on function public._report_digital_adoption(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_health_alerts
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_event_id is null then
    select e.id
      ...
      from public.events e
     where e.tenant_id = v_tenant
    and  exists (select 1 from public.event_registrations er where er.tenant_id = v_tenant
...
grant execute on function public._report_health_alerts(jsonb) to anon;
grant execute on function public._report_health_alerts(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_parking_estimate
-- ---------------------------------------------------------------------------
...
grant execute on function public._report_parking_estimate(jsonb) to anon;
grant execute on function public._report_parking_estimate(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_quorum_official
-- ---------------------------------------------------------------------------
...
grant execute on function public._report_quorum_official(jsonb) to anon;
grant execute on function public._report_quorum_official(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_support_suggestions
-- ---------------------------------------------------------------------------
...
grant execute on function public._report_support_suggestions(jsonb) to anon;
grant execute on function public._report_support_suggestions(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_territory_indicators
-- ---------------------------------------------------------------------------
...
grant execute on function public._report_territory_indicators(jsonb) to anon;
grant execute on function public._report_territory_indicators(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_treasury_sla
-- ---------------------------------------------------------------------------
...
begin
  ...
  base as (
    ...
    cross join bounds b
    where er.tenant_id = v_tenant
    and  er.created_at::date between b.month_start and b.month_end
  )
  select
    coalesce(jsonb_agg(
      jsonb_build_object(
...
grant execute on function public._report_treasury_sla(jsonb) to anon;
grant execute on function public._report_treasury_sla(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- _report_volunteer_engagement
-- ---------------------------------------------------------------------------
...
begin
  ...
  counts as (
    ...
    and  el.data_servico between b.month_start and b.month_end
...
grant execute on function public._report_volunteer_engagement(jsonb) to anon;
grant execute on function public._report_volunteer_engagement(jsonb) to authenticated;


-- ---------------------------------------------------------------------------
-- gerar_relatorio_manutencao
-- ---------------------------------------------------------------------------
