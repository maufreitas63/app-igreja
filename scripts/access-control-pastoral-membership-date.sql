-- Data de filiação (membership_date) na Mudança de Papéis.
-- Execute no SQL Editor do Supabase após access-control-pastoral-role-change.sql
-- e rename-profiles-baptism-date-to-membership-date.sql (se ainda não aplicado).
...
grant execute on function public.atualizar_membership_date_perfil_pastoral(uuid, uuid, date)
