-- Isolamento estrito: relatórios de manutenção só leem a instância da sessão.
-- SECURITY DEFINER ignora RLS; cada função precisa filtrar tenant_id = require_session_tenant_id().
-- Execute: npx supabase db query --linked -f scripts/maintenance-reports-tenant-isolation.sql

begin;

-- ---------------------------------------------------------------------------
-- Helper: membros/congregados ativos da instância da sessão
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 8. Inscritos por Evento (vazamento confirmado em produção)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Matriz etária (já filtrava; reafirma tenant)
-- ---------------------------------------------------------------------------
  ...
  if nullif(btrim(coalesce(p_params->>'event_id', '')), '') is not null then
    ...
    if not exists (
      ...
      return jsonb_build_object(
        'success', false,
        'report_code', 'health_alerts',
        'message', 'Evento não pertence a esta instância.'
      );
    end if;
  end if;
...
begin
  with base as (
    ...
    from public.events e
    join public.checkins c on c.event_id = e.id and c.tenant_id = v_tenant
    join public.profiles p on p.id = c.profile_id and p.tenant_id = v_tenant
    where e.tenant_id = v_tenant
...
-- ---------------------------------------------------------------------------
-- 6. Estimativa de estacionamento
-- ---------------------------------------------------------------------------
...
begin
  ...
  select
    ...
    jsonb_build_object(
      'evento', (
        select
          coalesce(nullif(trim(ev.name), ''), 'Evento sem nome')
          || case
            when ev.event_date is not null then ' — ' || to_char(ev.event_date::date, 'DD/MM/YYYY')
            else ''
          end
...
-- ---------------------------------------------------------------------------
-- Faixa etária (dispatcher ainda expõe demographic_age_brackets)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Fluxo financeiro (resumo sem tenant)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Retenção / absenteísmo (não está no catálogo de 8, mas a RPC existe)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Voluntários: tipos_escala do resumo sem tenant
-- ---------------------------------------------------------------------------
...
begin
  ...
  counts as (
    ...
    join public.voluntarios_escala ve on ve.id = el.voluntario_id and ve.tenant_id = v_tenant
    join public.tipos_escala te on te.id = el.tipo_escala_id and te.tenant_id = v_tenant
