...
import { FINANCIAL_MAX_RECEIPTS_PER_ENTRY } from '@/lib/financialReceiptUrls';
import {
  buildFinancialReferencia,
  parseTreasuryReceiptFileName,
  type ParsedTreasuryReceiptFileName,
} from '@/lib/treasuryReceiptBatchPath';

export type TreasuryReceiptBatchFileInput = {
  fileName: string;
  referencia: string;
  position: number;
  parsed: ParsedTreasuryReceiptFileName;
};

export type TreasuryReceiptBatchAmbiguousReferencia = {
  referencia: string;
  entries: FinancialEntry[];
};

export type TreasuryReceiptBatchPreflightIssue = {
  fileName: string;
  referencia?: string;
  position?: number;
  code:
...
export type TreasuryReceiptBatchPreflightResult = {
  valid: boolean;
  files: TreasuryReceiptBatchFileInput[];
  issues: TreasuryReceiptBatchPreflightIssue[];
  ambiguousReferencias: TreasuryReceiptBatchAmbiguousReferencia[];
  dateRange: { minIso: string; maxIso: string } | null;
};

const referenciaFromEntry = (entry: FinancialEntry) =>
  entry.referencia?.trim() || buildFinancialReferencia(entry.transaction_date, entry.amount);

export const buildReferenciaLookup = (entries: FinancialEntry[]) => {
...
export const listAmbiguousReferencias = (
  lookup: Map<string, FinancialEntry[]>
): TreasuryReceiptBatchAmbiguousReferencia[] => {
  const ambiguous: TreasuryReceiptBatchAmbiguousReferencia[] = [];
...
export const pickUniqueEntryForReferencia = (
  lookup: Map<string, FinancialEntry[]>,
  referencia: string
): { entry: FinancialEntry | null; ambiguous: boolean } => {
  const bucket = lookup.get(referencia);
...
const isoDateFromReferencia = (referencia: string) => {
...
export const extractReceiptBatchDateRange = (
  files: TreasuryReceiptBatchFileInput[]
...
export const filterEntriesByReceiptDateRange = (
  entries: FinancialEntry[],
  dateRange: { minIso: string; maxIso: string } | null
) => {
  if (!dateRange) {
...
const validatePositionSequence = (
  files: TreasuryReceiptBatchFileInput[],
  issues: TreasuryReceiptBatchPreflightIssue[]
) => {
  const byReferencia = new Map<string, TreasuryReceiptBatchFileInput[]>();
...
export const parseReceiptBatchFileInputs = (
  rawFileNames: string[]
): { files: TreasuryReceiptBatchFileInput[]; issues: TreasuryReceiptBatchPreflightIssue[] } => {
  const files: TreasuryReceiptBatchFileInput[] = [];
  const issues: TreasuryReceiptBatchPreflightIssue[] = [];
...
export const runTreasuryReceiptBatchPreflight = (
  rawFileNames: string[],
  entries: FinancialEntry[],
  options: { requireUniqueReferencia?: boolean } = {}
): TreasuryReceiptBatchPreflightResult => {
  ...
  const blockingCodes = new Set<TreasuryReceiptBatchPreflightIssue['code']>([
    'invalid_name',
    'duplicate_canonical',
    'position_gap',
    'ambiguous_referencia',
...
export const slotIsOccupied = (urls: string[], position: number) => position - 1 < urls.length;

export const entryHasRoomForReceipt = (urls: string[], force: boolean, position: number) => {
  if (force) {
