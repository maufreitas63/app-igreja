...
export type PttOpenConversationDetail = {
  ...
  /** Abertura pedida pelo usuário (retomar/nova) — sempre mostra o painel. */
  force?: boolean;
};

/** Abre o painel de diálogo PTT (ouvido pelo PttInboxListener). */
export function requestOpenPttConversation(conversationId: string, options?: { force?: boolean }) {
  ...
  window.dispatchEvent(
    new CustomEvent<PttOpenConversationDetail>(PTT_OPEN_CONVERSATION_EVENT, {
      detail: { conversationId: id, force: options?.force !== false },
    })
  );
}
